...
class OverlayService : Service() {
    ...
    private fun setupPersistentVirtualDisplay() {
        ...
        try {
            ...
            if (persistentVirtualDisplay == null) {
                ...
                Log.d(TAG, "VirtualDisplay created ${capturedWidth}x${capturedHeight}")
            }
        } catch (e: Exception) {
    ...
    private fun ensureCaptureSizeMatchesScreen() {
        ...
        try {
            ...
            Log.e(TAG, "Error resizing VirtualDisplay, recreating instead: ${e.message}", e)
            // resize ไม่ได้ในบางเครื่อง -> สร้างใหม่ทั้งก้อน (ยังใช้ mediaProjection เดิม ไม่ต้องขอสิทธิ์ใหม่)
            setupPersistentVirtualDisplay()
        }
    }
